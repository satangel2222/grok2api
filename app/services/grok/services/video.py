...
class VideoService:
    ...
    async def generate(
        ...
        message = await self._build_message(prompt, preset, nsfw_rewrite=nsfw_rewrite)
    ...
    async def generate_from_image(
        ...
        logger.info(
            f"Image to video: prompt='{prompt[:50]}...', image={image_url[:80]}, nsfw_rewrite={nsfw_rewrite}"
        )
            ...
            try:
                ...
                if image_attachments:
                    for attach_data in image_attachments:
                        # Skip re-upload if already on assets.grok.com
